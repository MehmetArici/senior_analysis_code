...
setwd('~/Dropbox/SpongeWebsite/analysis_code/correlation/')
filekey = "E-MTAB-2706"
file<- paste("/Volumes/normal/UAU/Projects/lncrnas_transfer/lncRNAs_Human/base_data/expression_data/", filekey, "-query-results_removedcomments.tsv", sep = "")
EMTAB_file = read.csv(file, header = TRUE, row.names=1, check.names=FALSE, sep = "\t")
...
#  nontarget_list = readLines(paste("/media/DATAPART1/lncRNAs_Human/analysis/separate_target_notarget_genes/results/code_v10_approach/nontargets_removedENCzeros.txt" , sep = ""))
  target_list = readLines('/Users/User/Dropbox/SpongeWebsite/Table2_target_notarget/results/PUM2_target_genes.txt')
  nontarget_list = readLines('/Users/User/Dropbox/SpongeWebsite/Table2_target_notarget/results/PUM2_nontarget_genes.txt')
...
#for (lncRNA in lncRNA_list){
  ...
  filename = paste('./results/', rbp, '_targets_nontargets_', 'lncRNAdata_', lncRNA, '_', filekey , '_', target_version, '.jpeg' , sep= '')
