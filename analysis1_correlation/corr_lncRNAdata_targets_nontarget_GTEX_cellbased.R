...
for (tissue in tissue_list){
  ...
  if (length(common_samples) != 0){
    ...
    if (mean(dataframe_a$value, na.rm = TRUE) < mean(dataframe_b$value, na.rm = TRUE)){
      print (paste(">>>>>", tissue, "<<<<<"))
    }
    ...
    filename = paste('/media/DATAPART1/PUM_analysis/correlation/results/corr_lncRNAdata/tissue_specific/', rbp, '_targets_nontargets_', 'lncRNAdata_', lncRNA, '_GTEX_', tissue , '.jpeg' , sep= '')
