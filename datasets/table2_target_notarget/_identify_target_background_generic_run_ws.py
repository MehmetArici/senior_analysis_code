...
dir = '/media/mllab/UAU/Projects/lncrnas_transfer/lncRNAs_Human/CLIP_datasets/'
...
finput_len = open ("/media/mllab/normal/UAU/Projects/lncrnas_transfer/lncrnas_eksik/ensembl_grch37_gene_transcript_length_sorted.txt")

finput_mapping = open ("/media/mllab/normal/UAU/Projects/lncrnas_transfer/PUM_analysis/base_data/ensembl_biomart_hg19_proteincoding_gene_refseq_mapping.txt")
...
for ensg_id in CLIP_file_all_ENSG_ids:
   if ensg_id in dict_length:
        ...
        if dict_count_signif_peaks[ensg_id] > 0 and dict_count_motifs_in_peaks[ensg_id] > 0:
           if ensg_id in dict_KD_K562 and dict_KD_K562[ensg] and ensg_id in dict_KD_HepG2 and dict_KD_HepG2[ensg]: 
               target_ids.append(ensg_id)
               target_ids_lens.append(dict_length[ensg_id])
        if dict_count_any_peak[ensg_id] and dict_motif[ensg_id] == 0: # no peak,  no motif (whether it's in the peak or not) at all..
           if ensg_id in dict_KD_K562 and not dict_KD_K562[ensg] and ensg_id in dict_KD_HepG2 and not dict_KD_HepG2[ensg]: # no signif LFC change
