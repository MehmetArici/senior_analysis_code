...
for ensg_id in CLIP_file_all_ENSG_ids:
   if ensg_id in dict_length:
        ...
        if dict_count_any_peak[ensg_id] > 0 and motif_check_target and KD_check_target:
              count_first_pass += 1
...
for i in range (start, num_first_bins):
	...
	if num_targets[i] != 0 and num_nontargets[i] != 0:
		...
		if num_targets[i] <= num_nontargets[i]:
			...
			sample = select_sample(dict_bins_targets[i], int(1.2 * len(dict_bins_nontargets[i])), 'target')
