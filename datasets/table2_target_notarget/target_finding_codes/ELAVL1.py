...
KDfile = "/Volumes/normal/UAU/Projects/lncrnas_transfer/lncRNAs_Human/base_data/Mukharjee_dataset.txt"
finput_kd_hur = open(KDfile)
...
dict_KD_hur = {}
for line in finput_kd_hur:
    words = line.strip("\n").split()
    nmid = words[0]
    lfc = float(words[1])
    if nmid in dict_mapping_nmid_to_ensg:
        ensg_id_kd = dict_mapping_nmid_to_ensg[nmid]
        if lfc < 0:
            dict_KD_hur[ensg_id_kd] = True
        if lfc > 2:
            dict_KD_hur[ensg_id_kd] = False   
...
for ensg_id in CLIP_file_all_ENSG_ids:
   if ensg_id in dict_length:
        ...
        motif_check_target = dict_motif[ensg_id] > 0
        ...
        KD_check_target = (ensg_id in dict_KD_hur)
        ...
        KD_check_nontarget = (ensg_id not in dict_KD_hur) or (not dict_KD_hur[ensg_id])     
