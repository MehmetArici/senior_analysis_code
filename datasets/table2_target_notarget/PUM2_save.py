...
ffimo = open('PUM2_top10_matches.txt')
...
list_activator = ["HNRNPL", "IGF2BP2", "IGF2BP3", "ELAVL1", "HuR"]

fout_target = open ('./results/' + RBP_OF_INTEREST + '_target_genes_top3.txt', 'w')

fout_nontarget = open ('./results/'  + RBP_OF_INTEREST + '_nontarget_genes_top3.txt', 'w')
...
for ensg_id in CLIP_file_all_ENSG_ids:
   if ensg_id in dict_length:
        ...
        #if dict_count_motifs_in_signif_peaks[ensg_id] > 0:
        #   count_second_pass += 1 
        if (ensg_id in dict_KD_K562 and dict_KD_K562[ensg_id]) or (ensg_id in dict_KD_HepG2 and dict_KD_HepG2[ensg_id]): 
            count_second_pass += 1  
        if dict_count_signif_peaks[ensg_id] > 0 and dict_count_motifs_in_signif_peaks[ensg_id] > 0:
           count_first_pass += 1
  
           if (ensg_id in dict_KD_K562 and dict_KD_K562[ensg_id]) or (ensg_id in dict_KD_HepG2 and dict_KD_HepG2[ensg_id]): 
           #if (ensg_id in dict_KD_K562_reverse and not dict_KD_K562_reverse[ensg_id]) and (ensg_id in dict_KD_HepG2_reverse and not dict_KD_HepG2_reverse[ensg_id]):
        ...
        if dict_count_any_peak[ensg_id] == 0 and (ensg_id not in dict_motif or dict_motif[ensg_id] == 0):
           if (K562_not_exist or (ensg_id in dict_KD_K562 and not dict_KD_K562[ensg_id])) and (HepG2_not_exist or (ensg_id in dict_KD_HepG2 and not dict_KD_HepG2[ensg_id])):  # no signif LFC change
               nontarget_ids.append(ensg_id)
               nontarget_ids_lens.append(dict_length[ensg_id])
   else:
       count_nolen += 1

print 'count_first_pass ' , count_first_pass, ' ', count_second_pass
print 'count_nolen ', count_nolen, 
